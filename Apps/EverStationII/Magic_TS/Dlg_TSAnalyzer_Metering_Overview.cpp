...
#include "..\resource.h"
/////////////////////////////////////////////////////////////////////////////
// CDlg_TSAnalyzer_Metering_Overview dialog

CDlg_TSAnalyzer_Metering_Overview::CDlg_TSAnalyzer_Metering_Overview(CWnd* pParent /*=NULL*/)
	: CDialog(CDlg_TSAnalyzer_Metering_Overview::IDD, pParent)
{
	//{{AFX_DATA_INIT(CDlg_TSAnalyzer_Metering_Overview)
...
void CDlg_TSAnalyzer_Metering_Overview::DoDataExchange(CDataExchange* pDX)
{
	CDialog::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(CDlg_TSAnalyzer_Metering_Overview)
...
BEGIN_MESSAGE_MAP(CDlg_TSAnalyzer_Metering_Overview, CDialog)
	//{{AFX_MSG_MAP(CDlg_TSAnalyzer_Metering_Overview)
	ON_WM_SIZE()
	//}}AFX_MSG_MAP
...
/////////////////////////////////////////////////////////////////////////////
// CDlg_TSAnalyzer_Metering_Overview message handlers

BOOL CDlg_TSAnalyzer_Metering_Overview::OnInitDialog() 
{
	...
	m_TsRateJitterGraphHistogram.Init_Y_Axis(AXIS_STYLE_CARTESIAN_FROM_MIN_TO_MAX, MEASURE_PANEL_HIDE | RANGE_MARK_HIDE, 0, 100, "%", 0, 100);	//X轴（-1000， 1000）bps，Y轴（0， 100%）比例			
...
void CDlg_TSAnalyzer_Metering_Overview::OnSize(UINT nType, int cx, int cy) 
{
...
void CDlg_TSAnalyzer_Metering_Overview::Reset(void)
{
	CWnd* pWnd = NULL;
...
void CDlg_TSAnalyzer_Metering_Overview::Set(int offline)
{
	CWnd* pWnd = NULL;
...
void CDlg_TSAnalyzer_Metering_Overview::UpdateSDT(CSDT* pSDT)
{
/*
...
void CDlg_TSAnalyzer_Metering_Overview::UpdateNIT(CNIT* pNIT)
{
	char	pszText[64];
...
void CDlg_TSAnalyzer_Metering_Overview::UpdatePAT(CPAT* pPAT)
{
	char		pszText[64];
	int			nCount;
	int			i;
...
void CDlg_TSAnalyzer_Metering_Overview::UpdatePacketLength(int packet_length)
{
	char					pszText[64];
	int						nCount;
	int						i;
...
void CDlg_TSAnalyzer_Metering_Overview::UpdateBitrateSample(int bitrate)
{
	if (m_nRunning == 1)
	{
...
void CDlg_TSAnalyzer_Metering_Overview::AppendEtr290Log(int type, const char* pszItem)
{
	CString			strTime;
...
void CDlg_TSAnalyzer_Metering_Overview::UpdatePacketInfo(void)
{
	int				nItemCount;
	int				item_index;
	//int				es_index;
...
void CDlg_TSAnalyzer_Metering_Overview::UpdatePMT(CPMT* pPMT)
{
	char			pszText[256];
	int				nItemCount;
	unsigned int	data;
...
void CDlg_TSAnalyzer_Metering_Overview::OnDestroy()
{
	CDialog::OnDestroy();
