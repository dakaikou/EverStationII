...
/////////////////////////////////////////////////////////////////////////////
// CDlg_TSAnalyzer_Metering_TSPackets dialog
#include "../Common/Define.h"

//#include "TSMagic_GuiApi.h"
#include "TSMagic_AnalyseThread_MSG.h"
#include "TSMagic_AnalyseThread_TSPacket_Decimate.h"
...
CDlg_TSAnalyzer_Metering_TSPackets::CDlg_TSAnalyzer_Metering_TSPackets(CWnd* pParent /*=NULL*/)
	: CDialog(CDlg_TSAnalyzer_Metering_TSPackets::IDD, pParent)
{
	//{{AFX_DATA_INIT(CDlg_TSAnalyzer_Metering_TSPackets)
	m_nStart = 0;
	m_nEnd = 0;
	m_nLength = 0;
	m_nTotal = 0;
...
void CDlg_TSAnalyzer_Metering_TSPackets::DoDataExchange(CDataExchange* pDX)
{
	CDialog::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(CDlg_TSAnalyzer_Metering_TSPackets)
...
BEGIN_MESSAGE_MAP(CDlg_TSAnalyzer_Metering_TSPackets, CDialog)
	//{{AFX_MSG_MAP(CDlg_TSAnalyzer_Metering_TSPackets)
	ON_WM_SIZE()
	...
	//}}AFX_MSG_MAP
	ON_BN_CLICKED(IDC_BTN_DECIMATE_STOP, &CDlg_TSAnalyzer_Metering_TSPackets::OnBnClickedBtnDecimateStop)
	ON_NOTIFY(NM_RCLICK, IDC_LIST_PID, &CDlg_TSAnalyzer_Metering_TSPackets::OnRclickListPid)
	ON_COMMAND(ID_TS_DECIMATE_TO_ES, &CDlg_TSAnalyzer_Metering_TSPackets::OnTsDecimateToEs)
	ON_COMMAND(ID_TS_DECIMATE_TO_PES, &CDlg_TSAnalyzer_Metering_TSPackets::OnTsDecimateToPes)
	ON_COMMAND(ID_TS_DECIMATE_TO_TS, &CDlg_TSAnalyzer_Metering_TSPackets::OnTsDecimateToTs)
	ON_WM_DESTROY()
END_MESSAGE_MAP()

/////////////////////////////////////////////////////////////////////////////
// CDlg_TSAnalyzer_Metering_TSPackets message handlers

void CDlg_TSAnalyzer_Metering_TSPackets::OnSize(UINT nType, int cx, int cy)
{
...
void CDlg_TSAnalyzer_Metering_TSPackets::InitPIDList(void)
{
...
BOOL CDlg_TSAnalyzer_Metering_TSPackets::OnInitDialog()
{
...
void CDlg_TSAnalyzer_Metering_TSPackets::UpdatePIDList(void)
{
...
void CDlg_TSAnalyzer_Metering_TSPackets::Reset(void)
{
	CWnd*	pWnd;
...
void CDlg_TSAnalyzer_Metering_TSPackets::Set(int offline)
{
	CWnd*	pWnd;
...
void CDlg_TSAnalyzer_Metering_TSPackets::ReportFileSize(__int64 filelength)			//unit: K 
{
	CWnd*	pWnd;
	CString str;
	int		page;
...
void CDlg_TSAnalyzer_Metering_TSPackets::OnBtnDecimateTS2TS()
{
...
//			rtcode = pDB_TSPackets->GetRecord(usPID, &TSPacketInfo);
	...
	if (nSelCount > 0)
	{
			...
			(pTSMagicView->m_kThreadParams.dsmcc_download_thread_running == 1))
		{
			...
			{
				sprintf_s(pszText, sizeof(pszText), "TS包录制：未能启动，因为发现PES捕捉线程尚未结束！");
				::SendMessage(pTSMagicView->GetSafeHwnd(), WM_TSMAGIC_APPEND_LOG, (WPARAM)pszText, (LPARAM)DEBUG_ERROR);
			}
...
void CDlg_TSAnalyzer_Metering_TSPackets::OnBtnDecimateTS2PES()
{
	// TODO: 在此添加控件通知处理程序代码
/*
	int		nSel;
...
void CDlg_TSAnalyzer_Metering_TSPackets::OnBtnDecimateTS2ES()
{
	// TODO: Add your control notification handler code here
...
//	int		action = 0;
	...
	if (nSel >= 0)
	{
			...
			(pTSMagicView->m_kThreadParams.dsmcc_download_thread_running == 1))
		{
			...
			if (pTSMagicView->m_kThreadParams.pes_trigger_thread_running == 1)
			{
				sprintf_s(pszText, sizeof(pszText), "TS包录制(TS->ES)：未能启动，因为发现PES捕捉线程尚未结束！");
				::SendMessage(pTSMagicView->GetSafeHwnd(), WM_TSMAGIC_APPEND_LOG, (WPARAM)pszText, (LPARAM)DEBUG_ERROR);
			}
			if (pTSMagicView->m_kThreadParams.section_trigger_thread_running == 1)
			{
...
void CDlg_TSAnalyzer_Metering_TSPackets::OnBnClickedBtnDecimateStop()
{
	// TODO: 在此添加控件通知处理程序代码
//	int		nSelCount;
	WORD	usPID;
...
void CDlg_TSAnalyzer_Metering_TSPackets::OnHScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar)
{
...
void CDlg_TSAnalyzer_Metering_TSPackets::OnBtnLeftmark()
{
	// TODO: Add your control notification handler code here
	m_nLeftMark = m_nStart;
...
void CDlg_TSAnalyzer_Metering_TSPackets::OnBtnRightmark()
{
	// TODO: Add your control notification handler code here
	m_nRightMark = m_nEnd;
...
void CDlg_TSAnalyzer_Metering_TSPackets::OnChangeEditStart()
{
...
void CDlg_TSAnalyzer_Metering_TSPackets::OnChangeEditLength()
{
	// TODO: If this is a RICHEDIT control, the control will not
	// send this notification unless you override the CDialog::OnInitDialog()
	// function and call CRichEditCtrl().SetEventMask()
...
void CDlg_TSAnalyzer_Metering_TSPackets::OnChangeEditEnd()
{
	// TODO: If this is a RICHEDIT control, the control will not
	// send this notification unless you override the CDialog::OnInitDialog()
	// function and call CRichEditCtrl().SetEventMask()
...
void CDlg_TSAnalyzer_Metering_TSPackets::ActionMsg(WPARAM wParam)
{
	CTSMagicView* pTSMagicView = CTSMagicView::GetView();
	ts_thread_params_t*	pthread_params = &(pTSMagicView->m_kThreadParams);
	CWnd* pWnd = NULL;
...
void CDlg_TSAnalyzer_Metering_TSPackets::OnRclickListPid(NMHDR *pNMHDR, LRESULT *pResult)
{
...
void CDlg_TSAnalyzer_Metering_TSPackets::OnTsDecimateToEs()
{
	// TODO: 在此添加命令处理程序代码
	OnBtnDecimateTS2ES();
}
...
void CDlg_TSAnalyzer_Metering_TSPackets::OnTsDecimateToPes()
{
	// TODO: 在此添加命令处理程序代码
	OnBtnDecimateTS2PES();
}
...
void CDlg_TSAnalyzer_Metering_TSPackets::OnTsDecimateToTs()
{
	// TODO: 在此添加命令处理程序代码
	OnBtnDecimateTS2TS();
}
...
void CDlg_TSAnalyzer_Metering_TSPackets::OnDestroy()
{
	CDialog::OnDestroy();
