...
void CDlg_TSAnalyzer_PesEs::DisplayMPVPacket(uint8_t* es_buf, int es_size, HALForXMLDoc* pxmlDoc, XMLElement* pxmlParentNode)
{
	...
	if ((es_buf != NULL) && (es_size > 0) && (pxmlDoc != NULL))
	{
		...
		{
			pxmlPESPayloadNode->SetAttribute("comment", pszField);
		}
		...
		do
		{
			...
			if (align_offset >= 0)
			{
				...
				default:
					...
					//	sprintf_s(pszComment, sizeof(pszComment), "%d字节 - vertical_postion 0x%02X~0x%02X", slices_length, slices_start_code, start_code);
