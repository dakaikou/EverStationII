...
//#include "GuiWnd_ListView_ServiceInfo.h"
...
IMPLEMENT_DYNCREATE(CNaviList_DsmccGuide, CListView)

CNaviList_DsmccGuide::CNaviList_DsmccGuide()
{
	m_hwndReceiver = NULL;
}
...
BEGIN_MESSAGE_MAP(CNaviList_DsmccGuide, CListView)
	//{{AFX_MSG_MAP(CListView_ServiceInfo)
	ON_WM_CREATE()
	ON_WM_SIZE()
	//}}AFX_MSG_MAP
	ON_NOTIFY_REFLECT(NM_DBLCLK, &CNaviList_DsmccGuide::OnNMDblclk)
	ON_NOTIFY_REFLECT(NM_RCLICK, &CNaviList_DsmccGuide::OnNMRClick)
	ON_COMMAND(ID_OCDC_DOWNLOAD, &CNaviList_DsmccGuide::OnOcdcDownload)
END_MESSAGE_MAP()

/////////////////////////////////////////////////////////////////////////////
// CListView_ServiceInfo drawing

void CNaviList_DsmccGuide::OnDraw(CDC* pDC)
{
...
#ifdef _DEBUG
void CNaviList_DsmccGuide::AssertValid() const
{
	CListView::AssertValid();
}

void CNaviList_DsmccGuide::Dump(CDumpContext& dc) const
{
	CListView::Dump(dc);
}
#endif //_DEBUG
...
BOOL CNaviList_DsmccGuide::Create(LPCTSTR lpszClassName, LPCTSTR lpszWindowName, DWORD dwStyle, const RECT& rect, CWnd* pParentWnd, UINT nID, CCreateContext* pContext) 
{
...
void CNaviList_DsmccGuide::Set(HWND hwndReceiver, int offline)
{
	m_hwndReceiver = hwndReceiver;
}

void CNaviList_DsmccGuide::Reset(void)
{
...
int CNaviList_DsmccGuide::OnCreate(LPCREATESTRUCT lpCreateStruct) 
{
...
void CNaviList_DsmccGuide::OnSize(UINT nType, int cx, int cy)
{
...
void CNaviList_DsmccGuide::UpdatePMT(CPMT* pPMT)
{
	char			pszTemp[64];
	int				bfound;
	int				insert_index;
...
void CNaviList_DsmccGuide::UpdateSDT(CSDT* pSDT)
{
	int				service_index;
	int				nCount;
	int				nItem;
...
void CNaviList_DsmccGuide::UpdateDSMCC(CPVT* pPVT)
{
	char			pszTemp[64];
...
void CNaviList_DsmccGuide::OnNMDblclk(NMHDR *pNMHDR, LRESULT *pResult)
{
...
void CNaviList_DsmccGuide::OnNMRClick(NMHDR *pNMHDR, LRESULT *pResult)
{
	LPNMITEMACTIVATE pNMItemActivate = reinterpret_cast<LPNMITEMACTIVATE>(pNMHDR);
	// TODO: 在此添加控件通知处理程序代码
	CMenu	menu, *pSubMenu;
...
void CNaviList_DsmccGuide::OnOcdcDownload(void)
{
	// TODO: 在此添加命令处理程序代码
	int		item_index;
	int		item_count;
