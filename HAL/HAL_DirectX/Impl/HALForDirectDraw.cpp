...
#include "../Include/HALForDirectDraw.h"

CHALForDirectDraw::CHALForDirectDraw(void)
{
	m_hVidWnd = NULL;
...
CHALForDirectDraw::~CHALForDirectDraw()
{
	assert(m_lpDD == NULL);
...
S32 CHALForDirectDraw::OpenVideo(HWND hWnd, S32 luma_width, S32 luma_height, char* pszFourCC, float frame_rate)
{
	S32 rtcode = -1;
...
S32 CHALForDirectDraw::CloseVideo(void)
{
	if (m_lpDDSOverlay)
	{
		m_lpDDSOverlay->Release();
...
S32 CHALForDirectDraw::RenderYUV(LPBYTE lpY, S32 y_size, LPBYTE lpU, S32 u_size, LPBYTE lpV, S32 v_size)
{
	LPBYTE  lpSurf = NULL;
	int		i;
	HRESULT	ddRval = -1;
		...
		(lpY != NULL) && (lpU != NULL) && (lpV != NULL))
	{
		do
		{
			...
			if (ddRval == DD_OK)
			{
				...
				//	if (dwTimeDiff >= 1000)
				//	{
				//		U32 fps = (U32)(m_nDebugFrameCount * 1000000 / dwTimeDiff);
...
S32 CHALForDirectDraw::RePaint(void)
{
	HRESULT	ddRval = -1;
	HWND	hWnd = m_hVidWnd;
