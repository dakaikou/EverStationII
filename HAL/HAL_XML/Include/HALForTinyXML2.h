...
#define XMLDOC_SetOriginBuf(pxmlDoc,buf) {if (pxmlDoc != NULL) pxmlDoc->SetOriginPtr(buf);}
...
public:
	XMLElement * NewElementForBits(XMLElement* pxmlParent, const char* key_name, uint32_t key_value, int bits = -1, const char* mnemonic = NULL, const char* pszComment = NULL, const BITS_t* pbits_map = NULL);
	XMLElement * NewElementForBytes(XMLElement* pxmlParent, const char* key_name, const uint8_t* byte_buf, int byte_length, const char* pszComment = NULL, const BITS_t* pbits_map = NULL);
	...
	//void UpdateBufMark(XMLElement* pxmlNode, uint8_t* start_ptr, uint8_t* end_ptr);
};
