...
void CAESDecoder::OpenAudio(HWND hWnd)
{
	m_hAudWnd = hWnd;
...
void CAESDecoder::CloseAudio(void)
{
	if (m_AudDecodeInfo.getparams == 1)		//initialize direct audio only when parameters are successfully got
	{
		DirectSound_Term();
...
#define SAVE_META_DATA_TO_DISK			0
...
BOOL CAESDecoder::DirectSound_AppData(LPDIRECTSOUNDBUFFER8 lpDsb,						 // The buffer.
...
S32 CAESDecoder::DirectSound_Init(void)
{
//	DSCAPS					 dscaps; 
	WAVEFORMATEX			 wfx; 
	...
	wfx.nChannels = m_AudDecodeInfo.nch; 
	wfx.nSamplesPerSec = m_AudDecodeInfo.sampling_rate; 
	wfx.wBitsPerSample = m_AudDecodeInfo.bitspersample; 
...
//					TRACE("DirectSound SetStopNotification Success.\n");
	...
	else
	{
		...
		{
		case DSERR_NODRIVER:
			//MessageBox(m_hAudWnd, "没有正确安装声卡驱动", "DirectSound Error", MB_OK);
			assert(0);
			break;
		default:
			// Add error-handling here.
			//MessageBox(m_hAudWnd, "DirectSound Create Error", "DirectSound Error", MB_OK);
			assert(0);
			break;
		}
	}
...
S32 CAESDecoder::DirectSound_Reset(void)
{
...
S32 CAESDecoder::DirectSound_Term(void)
{
	int	 i;
...
S32 CAESDecoder::DirectSound_Render(U8* buf, S32 length)
{
