...
#include "../../Include/xml/ac3_audio_sequence_xml.h"

#ifndef min
#define min(a,b)  (((a)<(b))?(a):(b))
#endif

int ac3_decode_unaligned_nal_to_xml(uint8_t* nal_buf, int nal_length, XMLDocForMpegSyntax* pxmlDoc, tinyxml2::XMLElement* pxmlParentNode)
{
...
int ac3_decode_unknown_nal_to_xml(uint8_t* nal_buf, int nal_length, XMLDocForMpegSyntax* pxmlDoc, tinyxml2::XMLElement* pxmlParentNode)
{
	int rtcode = AC3_NO_ERROR;
	char			pszTemp[64];
...
int	ac3_decode_syncinfo_to_xml(uint8_t* syncinfo_buf, int syncinfo_length, XMLDocForMpegSyntax* pxmlDoc, AC3_syncinfo_t* pac3_syncinfo)
{
	BITS_t	bs;
...
int	ac3_decode_syncframe_to_xml(uint8_t* es_frame_buf, int es_frame_length, XMLDocForMpegSyntax* pxmlDoc, tinyxml2::XMLElement* pxmlParentNode, AC3_syncframe_t* pAC3Syncframe)
{
	BITS_t	bs;
	int		rtcode = AC3_UNKNOWN_ERROR;
	int     header_size = 4;
