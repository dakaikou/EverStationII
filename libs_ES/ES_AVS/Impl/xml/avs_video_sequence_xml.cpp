...
#include "../../Include/xml/avs_video_sequence_xml.h"
...
int	avs_decode_pb_picture_header_to_xml(uint8_t* nal_buf, int nal_length, AVS_global_param_t* pparam, XMLDocForMpegSyntax* pxmlDoc, tinyxml2::XMLElement* pxmlParentNode, AVS_pb_picture_header_t* pPBPictureHeader)
{
	int		rtcode = AVS_NO_ERROR;
	BITS_t		bs;
	char pszTemp[64];
...
int avs_decode_slice_to_xml(uint8_t* nal_buf, int nal_length, AVS_global_param_t* pparam, XMLDocForMpegSyntax* pxmlDoc, tinyxml2::XMLElement* pxmlParentNode, AVS_slice_t* pSliceParam)
{
