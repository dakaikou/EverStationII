...
#include "avs2_video_vlc.h"
#include "avs2_video_table.h"
...
//
//static U8*			render_avs_video_buf_rgb888;
//static S32			render_avs_video_buf_size;
//
//static S32			MbWidth;								//avs_video_sequence_header & sequence_extension	
//static S32			MbHeight;
//static S32			MbCount;
//static S32			HalfMbCount;
//
//static U8			PictureType;
//static U8			PictureStructure;
//static U8			PictureReferenceFlag;
//
//static S32			PreviousQP;
//static S32			CurrentQP;
//static U8			FixedQP;
//static S32			SkipMbCount;
//
//static S32			MbIndex;
//static S32			SkipModeFlag;
//
//static S32			MbType;
//static S32			MvNum;
//static S32			MbPredMode;
//static S32			MbWeightingFlag;
//
//static U8			PatternCode[12];
//static S16			QuantCoeffArray[64];
//static S32			alternate_scan = 0;
//
//static S32			NumberOfReference;				//max value = 4
//
...
//}
//
//void avs_video_open(S32* pwidth, S32* pheight, S8* pszFourCC, S32 size)
//{
//	::WaitForSingleObject(ghAccessEvent, INFINITE);
//
//	if (!gbInitDecoder)
...
//		}
//
//		pucForwardFrame[CC_Y] = (U8*)malloc(avs_decode_info.luma_buf_size);
...
//		}
//
//		pucForwardFrame[CC_Cb] = (U8*)malloc(avs_decode_info.chroma_buf_size);
...
//		}
//
//		pucForwardFrame[CC_Cr] = (U8*)malloc(avs_decode_info.chroma_buf_size);
...
//		}
//
//		pucBackwardFrame[CC_Y] = (U8*)malloc(avs_decode_info.luma_buf_size);
...
//		}
//
//		pucBackwardFrame[CC_Cb] = (U8*)malloc(avs_decode_info.chroma_buf_size);
...
//		}
//
//		pucBackwardFrame[CC_Cr] = (U8*)malloc(avs_decode_info.chroma_buf_size);
...
//		}
//
//		pucAuxFrame[CC_Y] = (U8*)malloc(avs_decode_info.luma_buf_size);
...
//		}
//
//		pucAuxFrame[CC_Cb] = (U8*)malloc(avs_decode_info.chroma_buf_size);
...
//		}
//
//		pucAuxFrame[CC_Cr] = (U8*)malloc(avs_decode_info.chroma_buf_size);
...
////		}
//
////		render_avs_video_buf_rgb888 = (U8*)malloc(render_avs_video_buf_size);
...
//}
//
//void avs_video_reset(S32* pwidth, S32* pheight)
//{
//	::WaitForSingleObject(ghAccessEvent, INFINITE);
//
//	if (gbInitDecoder)
...
//}
//
////S32 avs_video_check_padding_bits(FIFO_t* pbs)
////{
////	S32 rtcode = 0;
////	U32 nextcode = 0;
////
////	U8*	rdptr = pbs->rdptr + 1;
////
...
////}
//
////S32 avs_video_next_start_code(FIFO_t* pbs)
////{
////	S32		start_code = -1;
...
////}
//
////S32	avs_video_scaling_list(FIFO_t* pbs, U8* scalingList, S32 sizeOfScalingList, U8* useDefaultScalingMatrixFlag)
////{
////	S32 lastScale = 8;
////	S32 nextScale = 8;
////	S32 delta_scale;
...
//}
//
////S32 avs_video_decode_video_sequence_header(FIFO_t* pbs)
////{
////	::WaitForSingleObject(ghAccessEvent, INFINITE);
////
////	/*syntax part parse*/
...
////}
//
//S32 avs_video_decode_buffer_clear(void)
//{
//	S32		pixindex;
//	S32		row, col;
//
...
//}
//
////S32	avs_video_decode_i_picture_header(FIFO_t* pbs)
////{
...
////}
//
////S32	avs_video_decode_video_sequence_end(FIFO_t* pbs)
////{
////	::WaitForSingleObject(ghAccessEvent, INFINITE);
////
////	FIFO_Flush(pbs, 32);
...
//}
//
////S32	avs_video_decode_user_data(FIFO_t* pbs)
////{
////	::WaitForSingleObject(ghAccessEvent, INFINITE);
////
...
//
////此部分只跟显示有关,跟解码过程没有关系
////S32	avs_video_decode_sequence_display_extension(FIFO_t* pbs)
////{
////	::WaitForSingleObject(ghAccessEvent, INFINITE);
////
...
//}
//
////S32	avs_video_decode_copyright_extension(FIFO_t* pbs)
////{
////	::WaitForSingleObject(ghAccessEvent, INFINITE);
////
////	FIFO_Flush(pbs, 32);
...
//}
//
////S32	avs_video_decode_camera_parameters_extension(FIFO_t* pbs)
////{
////	::WaitForSingleObject(ghAccessEvent, INFINITE);
////
////	FIFO_Flush(pbs, 32);
...
//}
//
////S32	avs_video_decode_picture_display_extension(FIFO_t* pbs)
////{
////	::WaitForSingleObject(ghAccessEvent, INFINITE);
////
////	FIFO_Flush(pbs, 32);
...
//}
//
////S32	avs_video_decode_pb_picture_header(FIFO_t* pbs)
////{
////	::WaitForSingleObject(ghAccessEvent, INFINITE);
////
////	FIFO_Flush(pbs, 32);
...
////}
//
//S32 avs_video_reorder_display_frame(void)
//{
//	switch (PictureType)
//	{
//	case AVS_I_FRAME:
...
//}
//
//S32 avs_video_vlc_lookup(S32 table, S32 trans_coefficient, AVS_VLC_Table_t* pvlc_tab)
//{
//	if (pvlc_tab)
//	{
//		switch (table)
...
//}
//
////S32 avs_video_block(FIFO_t* pbs, S32 i)
////{
////	S32 rtcode = 0;
////	S32 trans_coefficient;
////	S32	escape_level_diff;
////	S32 CurrentVLCTable;
////	S32 maxAbsLevel = 0;
////	S32 absLevel;
////	S32	RefAbsLevel;
...
////}
//
//AVS_MB_INFO_t* avs_video_get_macroblock(S32 macroblockindex)
//{
//	return NULL;
//}
//
//S32 avs_video_lut_p_macroblock_type(S32 MbTypeIndex, S32* pMbType, S32* pMvNum, S32* pMbPredMode)
//{
//	switch (MbTypeIndex)
//	{
//	case 0:
...
//}
//
//S32 avs_video_lut_b_macroblock_type(S32 MbTypeIndex, S32* pMbType, S32* pMvNum, S32* pMbPredMode)
//{
//	switch (MbTypeIndex)
//	{
//	case 0:
...
//}
//
//S32	avs_video_reconstruct_block(S32 mb_row, S32 mb_col, S32 i)
//{
//	S32		 rtcode = 0;
//	S32		 v, u;
//	S32		 v_2;
...
//}
//
//S32 avs_video_skipped_macroblock(S32 mb_index)
//{
//	switch (PictureType)
//	{
//	case AVS_I_FRAME:
...
//}
//
////S32 avs_video_macroblock(FIFO_t* pbs)
////{
////	S32		rtcode = -1;
////	S32		i;
////	S32		MbTypeIndex;
...
////}
//
////S32 avs_video_decode_slice(FIFO_t* pbs)
////{
////	S32		 rtcode = -1;
////	S32		 i;
////
...
////}
//
//U8* avs_video_output(S32* pwidth, S32* pheight)
//{
