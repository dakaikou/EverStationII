...
static U8*			pucForwardFrame[3] = {NULL, NULL, NULL};							//0 -- Y, 1 -- Cb, 2 -- Cr
static U8*			pucBackwardFrame[3] = {NULL, NULL, NULL};
static U8*			pucAuxFrame[3] = {NULL, NULL, NULL};

static U8*			pucDisplayFrame[3] = {NULL, NULL, NULL};					//当前显示帧
static U8*			pucDecodeFrame[3] = {NULL, NULL, NULL};						//当前解码帧

//static U8*			render_h264_video_buf_rgb888;
//static S32			render_h264_video_buf_size;

static S32			gbInitDecoder = 0;
...
static S32		m_nDebugMBCount;

#endif

static S32		m_nDebugFrameCount;
...
static S32 h264_video_decode_buffer_clear(void)
{
...
static S32 h264_video_reorder_decode_frame(const S32 slice_type)
{
...
static S32 h264_video_reorder_display_frame(const S32 slice_type)
{
	switch (slice_type)
	{
	case H264_SLICE_I0:
...
void h264_video_open(S32* pwidth, S32* pheight, S8* pszFourCC, S32 strSize)
{
	::WaitForSingleObject(ghAccessEvent, INFINITE);
...
//			h264_decode_info.display_height = h264_decode_info.graph_height;
			...
			pucForwardFrame[CC_Y] = (U8*)malloc(h264_decode_info.luma_buf_size);
			...
			pucForwardFrame[CC_Cb] = (U8*)malloc(h264_decode_info.chroma_buf_size);
			...
			pucForwardFrame[CC_Cr] = (U8*)malloc(h264_decode_info.chroma_buf_size);
			...
			pucBackwardFrame[CC_Y] = (U8*)malloc(h264_decode_info.luma_buf_size);
			...
			pucBackwardFrame[CC_Cb] = (U8*)malloc(h264_decode_info.chroma_buf_size);
			...
			pucBackwardFrame[CC_Cr] = (U8*)malloc(h264_decode_info.chroma_buf_size);
			...
			pucAuxFrame[CC_Y] = (U8*)malloc(h264_decode_info.luma_buf_size);
			...
			pucAuxFrame[CC_Cb] = (U8*)malloc(h264_decode_info.chroma_buf_size);
			...
			pucAuxFrame[CC_Cr] = (U8*)malloc(h264_decode_info.chroma_buf_size);
...
//			render_h264_video_buf_rgb888 = (U8*)malloc(render_h264_video_buf_size);
...
void h264_video_reset(S32* pwidth, S32* pheight)
{
	S32			luma_buf_size;
	S32			luma_pix_count;
...
H264_MB_INFO_t* h264_video_get_macroblock(S32 macroblockindex)
{
	return NULL;
}

//返回值：下一个单元的类型。当前类型可由pnal_unit带回
//S32 h264_video_get_annexb_nal_unit(FIFO_t* pfifo, H264_nal_unit_t* pnal_unit)
//{
...
static S32 h264_video_more_rbsp_data(BITS_t* pbs)
{
	S32	rtcode = 0;
	S32	ctr_bit;
	S32 leftbyte;
...
static S32 h264_video_scaling_list(BITS_t* pbs, S16* scalingList, S32 sizeOfScalingList, S16* useDefaultScalingMatrixFlag)
{
	S32 lastScale = 8;
	S32 nextScale = 8;
	S32 delta_scale;
...
static S32 h264_video_vui_parameters(BITS_t* pbs, pH264_vui_parameters_t pvui_parameters)
{
...
S32 h264_video_decode_seq_parameter_set(H264_nal_unit_t* pnal_unit)
{
	S32		rtcode = 0;
	S32		i;
	BITS_t	bs;
	...
	S32		profile_idc;
	S32		constraint_set0_flag;
	S32		constraint_set1_flag;
	S32		constraint_set2_flag;
	S32		constraint_set3_flag;
	...
	if ((seq_parameter_set_id >= 0) && (seq_parameter_set_id < MAXSPS))
	{
		...
		else if (pSPS->pic_order_cnt_type == 1)
		{
			...
			for (i = 0; i < (S32)pSPS->num_ref_frames_in_pic_order_cnt_cycle; i++)
			{
...
S32 h264_video_decode_pic_parameter_set(H264_nal_unit_t* pnal_unit)
{
	S32		rtcode = 0;
	S32		iGroup;
	S32		i;
	BITS_t	bs;
	S32		pic_parameter_set_id;
	...
	if ((pic_parameter_set_id >= 0) && (pic_parameter_set_id < MAXPPS))
	{
		...
		if (pPPS->num_slice_groups_minus1 > 0)
		{
			...
			{
				for (iGroup = 0; iGroup <= (S32)pPPS->num_slice_groups_minus1; iGroup++)
				{
			...
			else if (pPPS->slice_group_map_type == 6)
			{
				...
				S32 bits = CeilLog2(pPPS->num_slice_groups_minus1 + 1);
				for (i = 0; i <= (S32)pPPS->pic_size_in_map_units_minus1; i++)
				{
...
static S32 h264_video_decode_pred_weight_table(BITS_t* pbs, H264_pred_weight_table_t* ppred_weight_table)
{
	S32 i;
	S32 j;
...
static S32 h264_video_decode_ref_pic_list_reordering(BITS_t* pbs, S32 slice_type, H264_ref_pic_list_reordering_t* pref_pic_list_reordering)
{
	S32		i;
...
static S32 h264_video_decode_dec_ref_pic_marking(BITS_t* pbs, S32 nal_unit_type, H264_dec_ref_pic_marking_t* pdec_ref_pic_marking)
{
	S32		i;
...
S32 h264_video_decode_slice_header(BITS_t* pbs, S32 nal_ref_idc, S32 nal_unit_type)
{
	S32		rtcode = 0;
	S32		bits;
	S32		slice_type; 
...
_inline S32 h264_video_decode_NextMbAddress(S32 n)
{
	S32		i;
...
static S32 h264_video_decode_residual_block_cabac(BITS_t* pbs, S32 mbindex, S32 i4x4Blkidx, S16* coeffLevel, S32 maxNumCoeff)
{
	S32 rtcode = 0;
...
static S32 h264_video_lookup_TotalCoeff_TrailingOnes(BITS_t* pbs, S32 vlcindex, S32* pTotalCoeff, S32* pTrailingOnes)
{
	S32 rtcode = 0;
	S32 coeff_token;
	S32	len;
...
static S32 h264_video_lookup_TotalZeros(BITS_t* pbs, S32 TotalCoeff, S32 maxNumCoeff)
{
	S32 total_zeros = 0;
	S32 i;
	S32 cycle;
...
static S32 h264_video_lookup_RunBefore(BITS_t* pbs, S32 zerosLeft)
{
	S32 run_before = 0;
	S32 i;
	S32 cycle;
...
static S32 h264_video_mb_predict_non_zero_code(S32 mbindex, S32 i4x4BlkIdx)
{
	S32		nC = 0;
	S32		nA;
	S32		nB;
...
static S32	h264_video_reconstruct_block(S32 mb_row, S32 mb_col, S32 i8x8, S32 i4x4, S16 QF[4][4])
{
	S32		 rtcode = 0;
	S32		 v, u;
	S32		 w = 0;
...
//						pix_value = (QF[v/2][u] > 255) ? 255 : ((QF[v/2][u] < 0) ? 0 : QF[v/2][u]);
						...
						{
							pucDecodeFrame[cc][pix_index] = (U8)pix_value;
						}
...
static S32 h264_video_4x4_LevelScale(S32 m, S32 i, S32 j, S32 mbIsInterFlag, S32 iYCbCr)
{
	S32	rtcode = 1;
	S32 weightScale;
	S32	normAdjust;
	S32	scalIndex = iYCbCr + ((mbIsInterFlag == 1) ? 3 : 0);
...
static S32 h264_video_scaling_and_transform(S32 MbPartPredMode, S32 i4x4BlkIdx, S32 sMbFlag, H264_macroblock_layer_t* pMB, S16 c[4][4], S16 r[4][4])
{
	S32	qP;
	S32	rtcode = 0;
	S32	i, j;
...
static S32 h264_video_inverse_scan(S16 coeffLevel[16], S16 c[4][4])
{
	S32		rtcode = 0;
	S32		u, v;
...
static S32 h264_video_decode_residual_block_cavlc(BITS_t* pbs, S32 mbindex, S32 i4x4BlkIdx, S16* coeffLevel, S32 maxNumCoeff)
{
	S32 rtcode = 0;
	S32	i;
	S32	nC;
...
static S32 h264_video_decode_residual(BITS_t* pbs, S32 slice_type, S32 mbindex)
{
	S32 rtcode = 0;
	S32 (*residual_block)(BITS_t* pbs, S32 mbindex, S32 i4x4Blkidx, S16* coeffLevel, S32 maxNumCoeff);		//定义函数指针
	S32	MbPartPredMode;
	S32	iCbCr;
	S32 i8x8;
	S32	i4x4;
...
static S32 h264_video_decode_sub_mb_pred_cavlc(BITS_t* pbs, S32 slice_type, S32 mb_type)
{
	S32 rtcode = 0;
...
static S32 h264_video_decode_sub_mb_pred_cabac(BITS_t* pbs, S32 slice_type, S32 mb_type)
{
	S32 rtcode = 0;
...
static S32 h264_vide_decode_mb_pred_cavlc(BITS_t* pbs, S32 slice_type, S32 mbindex)
{
	S32		rtcode = 0;
	S32		MbPartPredMode;
	S32		luma4x4BlkIdx;
	...
	S32		mbAddrA;
	S32		mbAddrB;
	S32		chromaIndex = H264_LUT_chromaindex[pCurSPS->chroma_format_idc];
	S32		dcPredModePredictedFlag = 0;
	S32		intraMxMPredModeA;
	S32		intraMxMPredModeB;
	S32		MbPartPredModeA;
	S32		MbPartPredModeB;
	S32		predIntra4x4PredMode;
...
static S32 h264_vide_decode_mb_pred_cabac(BITS_t* pbs, S32 slice_type, S32 mbindex)
{
	S32		rtcode = 0;
	S32		MbPartPredMode;
	S32		luma4x4BlkIdx;
...
static S32 h264_video_decode_macroblock_layer_cavlc(BITS_t* pbs, S32 slice_type, S32 CurrMbAddress)
{
	S32		rtcode = 0;
	S32		MbPartPredMode;
	S32		mb_type;
	...
	S32		index;
	S32		mbPartIdx;
	S32		noSubMbPartSizeLessThan8x8Flag;
...
static S32 h264_video_decode_macroblock_layer_cabac(BITS_t* pbs, S32 slice_type, S32 CurrMbAddress)
{
	S32		rtcode = -1;
/*
	S32		entropy_coding_mode_flag = pCurPPS->entropy_coding_mode_flag;
	S32		MbPartPredMode;
	S32		mb_type;
	S32		codeNum;
	S32		index;
...
S32 h264_video_decode_slice_data(BITS_t* pbs, S32 nal_unit_type)
{
	S32 rtcode = 0;
	S32 CurrMbAddr;
	S32 moreDataFlag;
...
S32	h264_video_decode_slice_layer_without_partitioning(H264_nal_unit_t* pnal_unit)
{
	S32				rtcode = 0;
	BITS_t			bs;
...
//S32	h264_video_decode_slice_data_partition_a_layer_rbsp(FIFO_t* pbs)
//{
////	U32	slice_id;
//
//	::WaitForSingleObject(ghAccessEvent, INFINITE);
...
S32	h264_video_decode_slice_data_partition_b_layer_rbsp(BITS_t* pbs)
{
	U32 slice_id;
...
S32	h264_video_decode_slice_data_partition_c_layer_rbsp(BITS_t* pbs)
{
	U32 slice_id;
...
//S32	h264_video_decode_sei_rbsp(FIFO_t* pbs)
//{
//	::WaitForSingleObject(ghAccessEvent, INFINITE);
//
//	FIFO_Flush(pbs, 8);
...
//S32	h264_video_decode_access_unit_delimiter_rbsp(FIFO_t* pbs)
//{
//	U8	primary_pic_type;
//
//	::WaitForSingleObject(ghAccessEvent, INFINITE);
...
//S32	h264_video_decode_end_of_seq_rbsp(FIFO_t* pbs)
//{
//	::WaitForSingleObject(ghAccessEvent, INFINITE);
//
//	FIFO_Flush(pbs, 8);
...
//S32	h264_video_decode_end_of_stream_rbsp(FIFO_t* pbs)
//{
//	::WaitForSingleObject(ghAccessEvent, INFINITE);
//
//	FIFO_Flush(pbs, 8);
...
//S32	h264_video_decode_filler_data_rbsp(FIFO_t* pbs)
//{
//	::WaitForSingleObject(ghAccessEvent, INFINITE);
//
...
//S32	h264_video_decode_seq_parameter_set_extension_rbsp(FIFO_t* pbs)
//{
//	::WaitForSingleObject(ghAccessEvent, INFINITE);
//
//	FIFO_Flush(pbs, 8);
...
U8* h264_video_output(S32* pwidth, S32* pheight)
{
