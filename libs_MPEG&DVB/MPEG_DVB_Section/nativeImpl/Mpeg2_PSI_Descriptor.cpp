...
#include "../nativeInclude/Mpeg2_PSI_Descriptor.h"
...
S32 MPEG2_PSI_decode_video_stream_descriptor(U8 *buf, S32 length, pvideo_stream_descriptor_t pvideo_stream_descriptor)
{
	S32		rtcode = 0;
...
//返回：LPVOID指针
//备注：2000.11 chendelin
S32 MPEG2_PSI_decode_audio_stream_descriptor(U8 *buf, S32 length, paudio_stream_descriptor_t paudio_stream_descriptor)
{
	S32		rtcode = 0;
...
//返回：LPVOID指针
//备注：2000.11 chendelin
S32 MPEG2_PSI_decode_hierarchy_descriptor(U8 *buf, S32 length, phierarchy_descriptor_t phierarchy_descriptor)
{
	S32		rtcode = 0;
...
//返回：LPVOID指针
//备注：2000.11 chendelin
S32 MPEG2_PSI_decode_data_stream_alignment_descriptor(U8* buf, S32 length, pdata_stream_alignment_descriptor_t pdata_stream_alignment_descriptor)
{
	S32		rtcode = 0;
...
S32 MPEG2_PSI_decode_target_background_grid_descriptor(U8* buf, S32 length, ptarget_background_grid_descriptor_t ptarget_background_grid_descriptor)
{
	if ((buf != NULL) &&
		(length >= 2) &&
...
//输入：buffer, 起始位置nIndex
//返回：LPVOID指针
S32 MPEG2_PSI_decode_video_window_descriptor(U8* buf, S32 length, pvideo_window_descriptor_t pvideo_window_descriptor)
{
	if ((buf != NULL) &&
		(length >= 2) &&
...
//返回：LPVOID指针
//备注：2000.11 chendelin
S32 MPEG2_PSI_decode_system_clock_descriptor(U8 *buf, S32 length, psystem_clock_descriptor_t psystem_clock_descriptor)
{
	if ((buf != NULL) &&
		(length >= 2) &&
...
//输入：buffer, 起始位置nIndex
//返回：LPVOID指针
S32 MPEG2_PSI_decode_multiplex_buffer_utilization_descriptor(U8 *buf, S32 length, pmultiplex_buffer_utilization_descriptor_t pmultiplex_buffer_utilization_descriptor)
{
	if ((buf != NULL) &&
		(length >= 3) &&
...
//输入：buffer, 起始位置nIndex
//返回：LPVOID指针
S32 MPEG2_PSI_decode_copyright_descriptor(U8* buf, S32 length, pcopyright_descriptor_t pcopyright_descriptor)
{
	S32	copy_length;
	S32 loop_length;
...
//返回：LPVOID指针
//备注：2000.11 chendelin
S32 MPEG2_PSI_decode_maximum_bitrate_descriptor(U8 *buf, S32 length, pmaximum_bitrate_descriptor_t pmaximum_bitrate_descriptor)
{
	if ((buf != NULL) &&
		(length >= 2) &&
...
//输入：buffer, 起始位置nIndex
//返回：LPVOID指针
S32 MPEG2_PSI_decode_private_data_indicator_descriptor(U8* buf, S32 length, pprivate_data_indicator_descriptor_t pprivate_data_indicator_descriptor)
{
	if ((buf != NULL) &&
		(length >= 2) &&
...
S32 MPEG2_PSI_decode_smoothing_buffer_descriptor(U8* buf, S32 length, psmoothing_buffer_descriptor_t psmoothing_buffer_descriptor)
{
...
//输入：buffer, 起始位置nIndex
//返回：LPVOID指针
S32 MPEG2_PSI_decode_STD_descriptor(U8* buf, S32 length, pSTD_descriptor_t pSTD_descriptor)
{
	if ((buf != NULL) &&
		(length >= 3) &&
...
//输入：buffer, 起始位置nIndex
//返回：LPVOID指针
S32 MPEG2_PSI_decode_IBP_descriptor(U8* buf, S32 length, pIBP_descriptor_t pIBP_descriptor)
{
	if ((buf != NULL) &&
		(length >= 2) &&
...
S32 DSMCC_decode_deferred_association_tags_descriptor(U8* buf, S32 length, pdeferred_association_tags_descriptor_t pdeferred_association_tags_descriptor)
{
	S32		rtcode = 0;
	S32		loop_length;
	U8		N1, N2;
...
//0x1B
S32 MPEG2_PSI_decode_MPEG4_video_descriptor(U8* buf, S32 length, pMPEG4_video_descriptor_t pMPEG4_video_descriptor)
{
	if ((buf != NULL) &&
		(length >= 3) &&
...
//0x1C
S32 MPEG2_PSI_decode_MPEG4_audio_descriptor(U8* buf, S32 length, pMPEG4_audio_descriptor_t pMPEG4_audio_descriptor)
{
	if ((buf != NULL) &&
		(length >= 3) &&
...
//  0x20
S32 MPEG2_PSI_decode_External_ES_ID_descriptor(U8* buf, S32 length, pExternal_ES_ID_descriptor_t pExternal_ES_ID_descriptor)
{
	if ((buf != NULL) &&
		(length >= 4) &&
...
//0x28
S32 MPEG2_PSI_decode_AVC_video_descriptor(U8* buf, S32 length, pAVC_video_descriptor_t pAVC_video_descriptor)
{
	if ((buf != NULL) &&
		(length >= 6) &&
...
//0x2A
S32 MPEG2_PSI_decode_AVC_timing_and_HRD_descriptor(U8* buf, S32 length, pAVC_timing_and_HRD_descriptor_t pAVC_timing_and_HRD_descriptor)
{
	if ((buf != NULL) &&
		(length >= 2) &&
