...
int MPEG_decode_TS_adaptation_field_XML(uint8_t *buf, int length, XMLDocForMpegSyntax* pxmlDoc, tinyxml2::XMLElement* pxmlParentNode, adaptation_field_t* pAdaptFieldParam)
{
	S32	 rtcode = TSPACKET_PARSE_NO_ERROR;
	U8*  old_buf = NULL;
	U8*  adaptation_start_ptr = NULL;
	U8*  adaptation_end_ptr = NULL;
	tinyxml2::XMLElement* pAdaptationNode = NULL;
	...
	if ((pxmlDoc != NULL) && (pxmlParentNode != NULL))
	{
		if ((buf != NULL) && (length < 188))
		{
			...
			pAdaptationNode = pxmlDoc->NewKeyValuePairElement(pxmlParentNode, "adaptation_field()");

			padaptation_field->adaptation_field_length = BITS_get(&bs, 8);
			pxmlDoc->NewKeyValuePairElement(pAdaptationNode, "adaptation_field_length", padaptation_field->adaptation_field_length, 8, "uimsbf", NULL, &bs);
			...
			if ((padaptation_field->adaptation_field_length >= 0) && (padaptation_field->adaptation_field_length < 183))
			{
				if (padaptation_field->adaptation_field_length > 0)
				{
					...
					pxmlDoc->NewKeyValuePairElement(pAdaptationNode, "discontinuity_indicator", padaptation_field->discontinuity_indicator, 1, "bslbf", NULL, &bs);

					padaptation_field->random_access_indicator = BITS_get(&bs, 1);
					pxmlDoc->NewKeyValuePairElement(pAdaptationNode, "random_access_indicator", padaptation_field->random_access_indicator, 1, "bslbf", NULL, &bs);

					padaptation_field->elementary_stream_priority_indicator = BITS_get(&bs, 1);
					pxmlDoc->NewKeyValuePairElement(pAdaptationNode, "elementary_stream_priority_indicator", padaptation_field->elementary_stream_priority_indicator, 1, "bslbf", NULL, &bs);

					padaptation_field->PCR_flag = BITS_get(&bs, 1);
					pxmlDoc->NewKeyValuePairElement(pAdaptationNode, "PCR_flag", padaptation_field->PCR_flag, 1, "bslbf", NULL, &bs);

					padaptation_field->OPCR_flag = BITS_get(&bs, 1);
					pxmlDoc->NewKeyValuePairElement(pAdaptationNode, "OPCR_flag", padaptation_field->OPCR_flag, 1, "bslbf", NULL, &bs);

					padaptation_field->splicing_point_flag = BITS_get(&bs, 1);
					pxmlDoc->NewKeyValuePairElement(pAdaptationNode, "splicing_point_flag", padaptation_field->splicing_point_flag, 1, "bslbf", NULL, &bs);

					padaptation_field->transport_private_data_flag = BITS_get(&bs, 1);
					pxmlDoc->NewKeyValuePairElement(pAdaptationNode, "transport_private_data_flag", padaptation_field->transport_private_data_flag, 1, "bslbf", NULL, &bs);

					padaptation_field->adaptation_field_extension_flag = BITS_get(&bs, 1);
					pxmlDoc->NewKeyValuePairElement(pAdaptationNode, "adaptation_field_extension_flag", padaptation_field->adaptation_field_extension_flag, 1, "bslbf", NULL, &bs);

					if (padaptation_field->PCR_flag)
					{
						padaptation_field->program_clock_reference_base_32_30 = BITS_get(&bs, 3);
						pxmlDoc->NewKeyValuePairElement(pAdaptationNode, "program_clock_reference_base_32_30", padaptation_field->program_clock_reference_base_32_30, 3, "uimsbf", NULL, &bs);
						padaptation_field->program_clock_reference_base_29_15 = BITS_get(&bs, 15);
						pxmlDoc->NewKeyValuePairElement(pAdaptationNode, "program_clock_reference_base_29_15", padaptation_field->program_clock_reference_base_32_30, 15, "uimsbf", NULL, &bs);
						padaptation_field->program_clock_reference_base_14_0 = BITS_get(&bs, 15);
						pxmlDoc->NewKeyValuePairElement(pAdaptationNode, "program_clock_reference_base_14_0", padaptation_field->program_clock_reference_base_14_0, 15, "uimsbf", NULL, &bs);

						padaptation_field->reserved0 = BITS_get(&bs, 6);
						pxmlDoc->NewKeyValuePairElement(pAdaptationNode, "reserved", padaptation_field->reserved0, 6, "bslbf", NULL, &bs);

						padaptation_field->program_clock_reference_extension = BITS_get(&bs, 9);
						pxmlDoc->NewKeyValuePairElement(pAdaptationNode, "program_clock_reference_extension", padaptation_field->program_clock_reference_extension, 9, "uimsbf", NULL, &bs);
					}
					/*
				...
				pxmlDoc->UpdateBufMark(pAdaptationNode, adaptation_start_ptr, adaptation_end_ptr);
			...
			{
				pxmlDoc->NewKeyValuePairElement(pxmlParentNode, "TS packet adaptation field syntax error !");
				rtcode = TSPACKET_PARSE_SYNTAX_ERROR;
			}
		}
		else
		{
			pxmlDoc->NewKeyValuePairElement(pxmlParentNode, "TS packet adaptation field parameter error !");
...
int MPEG_decode_TS_packet_XML(uint8_t *buf, int length, XMLDocForMpegSyntax* pxmlDoc, transport_packet_t* pTSPacketParam)
{
	...
	if (pxmlDoc != NULL)
	{
		...
		//根节点
		tinyxml2::XMLElement* pxmlRootNode = ((tinyxml2::XMLDocument*)pxmlDoc)->NewElement("transport_packet()");
		...
		if ((buf != NULL) && ((length == 188) || (length == 204)))
		{
			if ((buf[0] == 0x47) || (buf[0] == 0xB8))
			{
				...
				pxmlDoc->NewKeyValuePairElement(pxmlRootNode, "sync_byte", ptransport_packet->sync_byte, 8, "bslbf", NULL, &bs);

				ptransport_packet->transport_error_indicator = BITS_get(&bs, 1);
				pxmlDoc->NewKeyValuePairElement(pxmlRootNode, "transport_error_indicator", ptransport_packet->transport_error_indicator, 1, "bslbf", NULL, &bs);

				ptransport_packet->payload_unit_start_indicator = BITS_get(&bs, 1);
				pxmlDoc->NewKeyValuePairElement(pxmlRootNode, "payload_unit_start_indicator", ptransport_packet->payload_unit_start_indicator, 1, "bslbf", NULL, &bs);

				ptransport_packet->transport_priority = BITS_get(&bs, 1);
				pxmlDoc->NewKeyValuePairElement(pxmlRootNode, "transport_priority", ptransport_packet->transport_priority, 1, "bslbf", NULL, &bs);

				ptransport_packet->PID = BITS_get(&bs, 13);
				pxmlDoc->NewKeyValuePairElement(pxmlRootNode, "PID", ptransport_packet->PID, 13, "uimsbf", NULL, &bs);
				...
				pxmlDoc->NewKeyValuePairElement(pxmlRootNode, "transport_scrambling_control", ptransport_packet->transport_scrambling_control, 2, "bslbf", pszComment, &bs);
				...
				pxmlDoc->NewKeyValuePairElement(pxmlRootNode, "adaptation_field_control", ptransport_packet->adaptation_field_control, 2, "bslbf", pszComment, &bs);

				ptransport_packet->continuity_counter = BITS_get(&bs, 4);
				pxmlDoc->NewKeyValuePairElement(pxmlRootNode, "continuity_counter", ptransport_packet->continuity_counter, 4, "bslbf", NULL, &bs);
				...
				if (rtcode == TSPACKET_PARSE_NO_ERROR)
				{
					if ((ptransport_packet->adaptation_field_control & 0b01) == 0b01)	//判断是否有净荷
					{
						...
						if (ptransport_packet->payload_length > 0)
						{
							...
							pxmlDoc->NewKeyValuePairElement(pxmlRootNode, "data_byte[ ]", ptransport_packet->payload_buf, ptransport_packet->payload_length, NULL, &bs);
						}
					}
					else
					{
			...
			{
				pxmlRootNode->SetAttribute("error", "buffer parameter error!");
				rtcode = TSPACKET_PARSE_SYNC_ERROR;
			}
		}
		else
		{
			pxmlRootNode->SetAttribute("error", "input parameter error!");
			rtcode = TSPACKET_PARSE_PARAMETER_ERROR;
		}
	}
	else
