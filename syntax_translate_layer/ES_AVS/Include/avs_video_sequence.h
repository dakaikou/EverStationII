...
#   ifdef _AVS_EXPORT
#       define AVS_SYNTAX_LIB __declspec(dllexport)
#   elif defined(_AVS_IMPORT)
#       define AVS_SYNTAX_LIB __declspec(dllimport)
#   else
#       define AVS_SYNTAX_LIB
#   endif
#elif __GNUC__ >= 4
#   define AVS_SYNTAX_LIB __attribute__((visibility("default")))
#else
#   define AVS_SYNTAX_LIB
#endif
...
typedef struct
{
	...
	uint16_t		bbv_delay;										//u(16)
	uint8_t			marker_bit;										//f(1)
	uint8_t			bbv_delay_extension;							//u(7)   AVS+
	...
	uint8_t			no_forward_reference_flag;						//u(1)
	uint8_t			pb_filed_enhanced_flag;							//u(1)
	uint8_t			reserved_bits;									//u(4)
	uint8_t			skip_mode_flag;									//u(1)
	uint8_t			loop_filter_disable;							//u(1)
	uint8_t			loop_filter_parameter_flag;						//u(1)
