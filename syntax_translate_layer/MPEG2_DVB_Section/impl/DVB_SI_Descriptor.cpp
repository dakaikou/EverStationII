...
//输入：buffer, 起始位置nIndex
//返回：LPVOID指针
int DVB_SI_decode_satellite_delivery_system_descriptor(uint8_t* buf, int length, psatellite_delivery_system_descriptor_t psatellite_delivery_system_descriptor)
{
	if ((buf != NULL) &&
		(length >= 2) &&
...
//输入：buffer, 起始位置nIndex
//返回：LPVOID指针
int DVB_SI_decode_cable_delivery_system_descriptor(uint8_t* buf, int length, pcable_delivery_system_descriptor_t pcable_delivery_system_descriptor)
{
	if ((buf != NULL) &&
		(length >= 2) &&
...
//0x45
int	DVB_SI_decode_VBI_data_descriptor(uint8_t* buf, int length, pVBI_data_descriptor_t pVBI_data_descriptor)
{
	int		N = 0;
	uint8_t*		pend;
...
//输入：buffer, 起始位置nIndex
//返回：LPVOID指针
int DVB_SI_decode_country_availability_descriptor(uint8_t* buf, int length, pcountry_availability_descriptor_t pcountry_availability_descriptor)
{
	int loop_length;
	int N = 0;
...
//输入：buffer, 起始位置nIndex
//返回：LPVOID指针
int DVB_SI_decode_linkage_descriptor(uint8_t* buf, int length, plinkage_descriptor_t plinkage_descriptor)
{
	int		copy_length;
	int		N;
	int		M;
...
//输入：buffer, 起始位置nIndex
//返回：LPVOID指针
int DVB_SI_decode_NVOD_reference_descriptor(uint8_t* buf, int length, pNVOD_reference_descriptor_t pNVOD_reference_descriptor)
{
	int N = 0;
	int loop_length;
...
//输入：buffer, 起始位置nIndex
//返回：LPVOID指针
int DVB_SI_decode_time_shifted_service_descriptor(uint8_t* buf, int length, ptime_shifted_service_descriptor_t ptime_shifted_service_descriptor)
{
	if ((buf != NULL) &&
		(length >= 2) &&
...
//输入：buffer, 起始位置nIndex
//返回：LPVOID指针
int DVB_SI_decode_time_shifted_event_descriptor(uint8_t* buf, int length, ptime_shifted_event_descriptor_t ptime_shifted_event_descriptor)
{
	if ((buf != NULL) &&
		(length >= 2) &&
...
//输入：buffer, 起始位置nIndex
//返回：LPVOID指针
int DVB_SI_decode_component_descriptor(uint8_t* buf, int length, pcomponent_descriptor_t pcomponent_descriptor)
{
	int rtcode = SECTION_PARSE_UNKNOWN_ERROR;
	int	copy_length;
	int text_length;
...
//输入：buffer, 起始位置nIndex
//返回：LPVOID指针
int DVB_SI_decode_mosaic_descriptor(uint8_t* buf, int length, pmosaic_descriptor_t pmosaic_descriptor)
{
	int loop_length;
	int	N = 0;
	int i;
...
//输入：buffer, 起始位置nIndex
//返回：LPVOID指针
int DVB_SI_decode_content_descriptor(uint8_t* buf, int length, pcontent_descriptor_t pcontent_descriptor)
{
	int N = 0;
	int loop_length;
...
//输入：buffer, 起始位置nIndex
//返回：LPVOID指针
int DVB_SI_decode_telephone_descriptor(uint8_t* buf, int length, ptelephone_descriptor_t ptelephone_descriptor)
{
	if ((buf != NULL) &&
		(length >= 2) &&
...
//输入：buffer, 起始位置nIndex
//返回：LPVOID指针
int DVB_SI_decode_terrestrial_delivery_system_descriptor(uint8_t* buf, int length, pterrestrial_delivery_system_descriptor_t pterrestrial_delivery_system_descriptor)
{
	if ((buf != NULL) &&
		(length >= 2) &&
...
//输入：buffer, 起始位置nIndex
//返回：LPVOID指针
int DVB_SI_decode_service_move_descriptor(uint8_t* buf, int length, pservice_move_descriptor_t pservice_move_descriptor)
{
	if ((buf != NULL) &&
		(length >= 2) &&
...
//输入：buffer, 起始位置nIndex
//返回：LPVOID指针
int DVB_SI_decode_short_smoothing_buffer_descriptor(uint8_t* buf, int length, pshort_smoothing_buffer_descriptor_t pshort_smoothing_buffer_descriptor)
{
	if ((buf != NULL) &&
		(length >= 2) &&
...
//输入：buffer, 起始位置nIndex
//返回：LPVOID指针
int DVB_SI_decode_frequency_list_descriptor(uint8_t* buf, int length, pfrequency_list_descriptor_t pfrequency_list_descriptor)
{
	int N = 0;
	int loop_length = 0;
...
//输入：buffer, 起始位置nIndex
//返回：LPVOID指针
int DVB_SI_decode_partial_transport_stream_descriptor(uint8_t* buf, int length, ppartial_transport_stream_descriptor_t ppartial_transport_stream_descriptor)
{
	if ((buf != NULL) &&
		(length >= 8) &&
...
//输入：buffer, 起始位置nIndex
//返回：LPVOID指针
//int SI_decode_CA_system_descriptor(uint8_t* buf, int length, pCA_system_descriptor_t pCA_system_descriptor)
int DVB_SI_decode_scrambling_descriptor(uint8_t* buf, int length, pscrambling_descriptor_t pscrambling_descriptor)
{
	int	rtcode = 0;
...
//输入：
//返回：
int DVB_SI_decode_ac3_descriptor(uint8_t* buf, int length, pac3_descriptor_t pac3_descriptor)
{
	int		rtcode = SECTION_PARSE_NO_ERROR;
	uint8_t*		porigin = buf;
	uint8_t		N;
...
//输入：
//返回：
int DVB_SI_decode_application_signalling_descriptor(uint8_t* buf, int length, papplication_signalling_descriptor_t papplication_signalling_descriptor)
{
	int		rtcode = 0;
	uint8_t*		porigin = buf;
	uint8_t		i;
...
//0x6C
int DVB_SI_decode_cell_list_descriptor(uint8_t* buf, int length, pcell_list_descriptor_t pcell_list_descriptor)
{
	uint8_t*		porigin = buf;
	uint8_t*		pend;
	uint8_t*		pnext;
...
//0x6D
int DVB_SI_decode_cell_frequency_link_descriptor(uint8_t* buf, int length, pcell_frequency_link_descriptor_t pcell_frequency_link_descriptor)
{
	int		rtcode = SECTION_PARSE_NO_ERROR;
	uint8_t*		porigin = buf;
	uint8_t*		pend;
...
//0x77
int DVB_SI_decode_time_slice_fec_identifier_descriptor(uint8_t* buf, int length, ptime_slice_fec_identifier_descriptor_t ptime_slice_fec_identifier_descriptor)
{
	int		copy_length;
...
//0x7F
int DVB_SI_decode_extension_descriptor(uint8_t* buf, int length, pextension_descriptor_t pextension_descriptor)
{
...
//输入：
//返回：
int DVB_SI_decode_AC3_audio_stream_descriptor(uint8_t* buf, int length, pAC3_audio_stream_descriptor_t pAC3_audio_stream_descriptor)
{
	uint8_t*		porigin = buf;
...
//输入：
//返回：
int SI_decode_channel_order_descriptor(uint8_t* buf, int length, pchannel_order_descriptor_t pchannel_order_descriptor)
{
	uint8_t*		porigin = buf;
	uint8_t		i;
...
//输入：
//返回：
int SI_decode_channel_volume_compensating_descriptor(uint8_t* buf, int length, pchannel_volume_compensating_descriptor_t pchannel_volume_compensating_descriptor)
{
	uint8_t*		porigin = buf;
	uint8_t		i;
...
//输入：
//返回：
int SI_decode_channel_classify_descriptor(uint8_t* buf, int length, pchannel_classify_descriptor_t pchannel_classify_descriptor)
{
	uint8_t*		porigin = buf;
	uint8_t		i;
