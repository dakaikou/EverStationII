...
------------------------------------------------------------------------------------------------*/
static int AC3_exp_unpack_ch(uint16_t type, uint8_t expstr, uint8_t ngrps, S8 initial_exp, S8 exps[], uint16_t *dest)
{
	uint16_t i,j;
	S16 exp_acc;
...
/*------------------------------------------------------------------------------------------------
	...
	维护：
			...
			2006.03.20	chendelin		从参考程序移植
------------------------------------------------------------------------------------------------*/
int AC3_exp_unpack(uint8_t expstr, S8 absexp, S8* exps, uint8_t ngrps, uint16_t *dest)
{
	uint16_t  i, j;
	int  grp;
	int	 grpoffset;
	...
	aexp = (uint16_t*)malloc((ngrps * 3) * sizeof(uint16_t));
...
/*------------------------------------------------------------------------------------------------
	...
	维护：
			...
			2006.03.20	chendelin		从参考程序移植
------------------------------------------------------------------------------------------------*/
int AC3_exponent_unpack(AC3_bsi_t *bsi, AC3_audblk_t *audblk)
{
	int error_flag = 0;
	int ch = 0;
